...
export class OrdersRepository {
  ...
  public static async findOrdersWithProducts(order_date: Date): Promise<ProductWithOrdersAndUser[]>{
    return new Promise((resolve, reject)=>{
      connection.query("SELECT orders_id, product_id, user_id, ProductOrders.amount, order_date, total_amount, status,description, price, name, lastname, email, number_phone FROM Orders JOIN User On user_id=user_id_fk JOIN ProductOrders ON orders_id= orders_id_fk JOIN Product ON product_id=product_id_fk WHERE order_date=? AND status = Vendido,  Orders.deleted=0 AND Product.deleted = 0 AND User.deleted =0 ORDER BY orders_id ", [order_date], (error: any, results)=>{
        if(error){
          reject(error);
        }else{
