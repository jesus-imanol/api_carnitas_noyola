...
export class OrdersRepository {
  ...
  public static async createOrder(order: Orders): Promise<Orders> {
    const query = 'INSERT INTO Product (order_date, total_amount, status, payment_method, created_at, created_by, updated_at, updated_by, deleted) VALUES (?,?, ?, ?, ?, ?, ?, ?,?)';
    return new Promise((resolve, reject) => {
  ...
  public static async updateOrder(orders_id: number, orderData: Orders): Promise<Orders | null> {
    const query = 'UPDATE Product SET order_date=?, total_amount=?, status=?, payment_method=?, created_at=?, created_by=?, updated_at=?, updated_by=?, deleted=? WHERE orders_id = ?';
    return new Promise((resolve, reject) => {
